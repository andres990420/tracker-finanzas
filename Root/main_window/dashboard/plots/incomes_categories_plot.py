...
class IncomesCategoriesPlot(QWidget):
    def __init__(self, year):
        ...
        sets_incomes_categories = list(self.sets.keys())
        incomes_categories_list = TransactionServices().get_incomes_categories_dashboard(
            Session.get_current_user_id(), self.year)
        months_list = []
        data_dict = {category: [] for category in sets_incomes_categories}

        for value, month, category in incomes_categories_list:
        ...
        self.chart.setTitle(f'INGRESOS POR CATEGORIA {self.year}')
        self.chart.legend().setAlignment(Qt.AlignmentFlag.AlignRight)
        self.chart.setAnimationOptions(QChart.AnimationOption.SeriesAnimations)

        self.categories = Utils.get_months(months_list)
        self.axis_x = QBarCategoryAxis()
        self.axis_x.append(self.categories)
        self.chart.addAxis(self.axis_x, Qt.AlignmentFlag.AlignBottom)
        self.series.attachAxis(self.axis_x)

        incomes_list_max = max([max(values) for values in data_dict.values() if any(values)])

        self.axis_y = QValueAxis()
        self.axis_y.setRange(0, incomes_list_max + 1000)
    ...
    def update_chart(self, year):
        ...
        self.chart.setTitle(f'INGRESO POR CATEGORIA {self.year}')
        ...
        new_category = Utils().get_months(months_list)
